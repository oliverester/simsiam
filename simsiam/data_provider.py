...
class DataProvider():
    ...
    def get_wsi_loaders(self) -> List[torch.utils.data.DataLoader]:
        """Generates a list of DataLoader - each representing all relevant patches from a WSI (generated by pre-histo)

        Returns:
            List[torch.utils.data.DataLoader]: list of DataLoader
        """
        
        wsi_loader_list = list()
        wsidir = self.args.test_data
        WSIs = WSIDatasetFolder(root_folder=wsidir).get_WSIs()

        for wsi in WSIs:
            wsi_dataset = CustomPatchesDataset(wsi_dataset=WSIDatasetFolder(root_folder=validir), 
                                            transform=self.non_aug_transform)
            # validation loader does not have distributed loader. So, each GPU runs a full validation run. However, only rank 0 prints
            wsi_loader = torch.utils.data.DataLoader(
            wsi_dataset,
            batch_size=256, shuffle=False,
            num_workers=self.args.workers, pin_memory=True)
            wsi_loader_list.append(wsi_loader)
            
        return wsi_loader_list
